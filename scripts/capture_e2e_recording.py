...
def run_subprocess(args: List[str], log_path: Path,
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    # 1. Real E2E suite
    e2e_args = [
        sys.executable, str(SCRIPTS / "run_real_e2e.py"),
        "--output-dir", str(out_root / "real-e2e"),
    ]
    if args.url_only:
    ...
    rc, out, err = run_subprocess(e2e_args, out_root / "real-e2e.log",
                                  timeout=args.timeout)
    summary["runs"].append({
        "name": "run_real_e2e", "rc": rc,
    ...
    if not args.url_only:
        wm_args = [
            sys.executable, str(SCRIPTS / "run_wechat_mp_e2e.py"),
            "--from-fixture",
            "--output-dir", str(out_root / "wechat-mp-e2e"),
            "--wechat-cookies", "wxuin=demo; pass_ticket=demo",
            "--dry-run",
        ]
        rc, out, err = run_subprocess(wm_args, out_root / "wechat-mp-e2e.log",
                                      timeout=args.timeout)
        summary["runs"].append({
            "name": "run_wechat_mp_e2e", "rc": rc,
            "ok_lines": sum(1 for line in out.splitlines() if line.startswith("[OK")),
            "fail_lines": sum(1 for line in out.splitlines() if line.startswith("[FAIL")),
            "skip_lines": sum(1 for line in out.splitlines() if line.startswith("[SKIP")),
        })
